...
import AfterImage_torch as aft
...
def incstatGetStats(incstat, t, v):
    ...
    if incstat.isTypeDiff:
        v = F.relu(t - incstat.lastTimestamp)
    factor = torch.pow(2, (-incstat.Lambda * F.relu(t - incstat.lastTimestamp)))
...
def getStats(nstat, srcMAC, dstMAC, srcIP, srcProtocol, dstIP, dstProtocol, datagramSize, timestamp):
    ...
    try:
        ...
        if srcProtocol == 'arp':
            ...
            res += incstatCovGetStats(nstat.HT_Hp.HT[srcIP + srcProtocol], nstat.HT_Hp.HT[dstIP + dstProtocol],
                                      timestamp,
                                      datagramSize)
...
def printIncstatCov(inc_cov):
    for incS in inc_cov.incStats:
        print(incS.ID, end="    ")
    print()
    print("lastRes: ", inc_cov.lastRes)
    print("CF3: ", inc_cov.CF3)
    print("w3: ", inc_cov.w3)
    print("lastTimestamp_cf3: ", inc_cov.lastTimestamp_cf3)

def getIncstat_cov(incstat1, incstat2):
    '''
    return incstat_cov object given incstat1 and incstat2
    '''
    for cov in incstat1.covs:
        if cov.incStats[0].ID == incstat2.ID or cov.incStats[1].ID == incstat2.ID:
            return cov
...
def printAll(incstat1, incstat2):
    print("*" * 30)
    printIncstat(incstat1)
    print("*" * 30)
    printIncstat(incstat2)
    incstat_cov = getIncstat_cov(incstat1, incstat2)
    if incstat_cov is not None:
        printIncstatCov(incstat_cov)
    else:
        print("incstat_cov is None")
    return incstat_cov
