...
class incStat:
    def __init__(self, Lambda, ID, init_time=0, isTypeDiff=False):  # timestamp is creation time
        ...
        self.CF1 = np.zeros(len(self.Lambda))  # linear sum
        self.CF2 = np.zeros(len(self.Lambda))  # sum of squares
        self.w = 1e-20 * np.ones(len(self.Lambda)) # weight
        self.isTypeDiff = isTypeDiff
        self.lastTimestamp = init_time
        self.cur_mean = np.zeros(1)
        self.cur_var = np.zeros(1)
        self.cur_std = np.zeros(1)
    ...
    def processDecay(self, timestamp):
        factor = 2 ** (-self.Lambda * relu(timestamp - self.lastTimestamp))
